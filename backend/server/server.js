...
const root = {
    ...
    user: async ({name, password}) => {
        const document = await userModel.findOne({name: name, password: password})
        console.log(document);
        return document;
      },
    ...
    addRoom: async({title, description, group, owner}) => {
      const document = new roomModel({ title: title, description: description, group: group, owner: owner});
      await document.save((err, room) => {
        if(err) return console.error(err);
        console.log(`Room ${room.title} group: ${room.group} added to database`);
      })
      return document;
    },
  ...
  updateRoom: async ({id, title, description, group, owner}) => {
    const update = {title: title, description: description, group: group, owner: owner};
    const document = await roomModel.findByIdAndUpdate({_id: id}, update, (err,doc) => {
